...
def get_engine(engine_path):
    ...
    with open(engine_path, "rb") as f, trt.Runtime(logger) as runtime:
        meta_len = int.from_bytes(f.read(4),
                                  byteorder="little")  # read metadata length
...
parser.add_argument('--output-file', type=str, help='output file name including directory name')
...
model.export(
    ...
    half=True,
    int8=False,
    data="train_full.yaml",
    device=DEVICE,
    simplify=True,
)
#
# Load the exported TensorRT model
model = YOLO(args.model_file.replace('pt', 'engine'), task="detect")
